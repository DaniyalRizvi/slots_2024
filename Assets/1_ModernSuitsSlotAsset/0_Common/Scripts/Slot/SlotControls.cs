...
namespace Mkey
{
    ...
    {
        [SerializeField] private GameObject MaxBetEffects;
        ...
        //public int TotalBet
        //{
        //    get { return LineBet * SelectedLinesCount * HoldMultipler * (MPlayer.Level + 1) * 100; }
        //}

        //public int TotalBet
        //{
        //    get { return LineBet * SelectedLinesCount * HoldMultipler * 1000; }
        //}
        ...
        public int TotalBet
        {
            get 
            {
                return TotalBetCalculator(LineBet);
            }
        }

        public int TotalBetCalculator(int lineBet)
        {
            int amount;
            switch(lineBet)
            {
        ...
        private IEnumerator Start()
        {
            ...
            maxLineBet = Math.Max(2, MPlayer.Level + 1);
            ...
            if (spinButtonBehavior)
            {
                ...
                stopButton.onClick.AddListener(() => Stop_Click());
            }
            ...
            LoadFreeSpins();
            LoadAutoSpins();
            SetTextString(InfoText, (SelectedLinesCount > 0) ? "Click SPIN to start!" : "Select any slot line!");
            ChangeSelectedLinesEvent += (l, b) => { SetTextString(InfoText, (l > 0) ? "Click SPIN to start!" : "Select any slot line!"); };
            Refresh();
        }
        ...
        public void UpdateMaxMetLine()
        {
            maxLineBet = Math.Max(2, MPlayer.Level + 1);
        }

        public int GetMaxMetLine()
        {
            return maxLineBet;
        }
        #region refresh
        ...
        {
            if (maxLineBet - LineBet == 1)
                MaxBetEffects.GetComponentInChildren<ParticleSystem>().Play();
            else if (maxLineBet == LineBet)
                SoundMaster.Instance.PlayBuzzSoundEffect();
            AddLineBet(1);
        }
        ...
        public void Fast_Click()
        {
            if(!isFast)
            {
                slot.SetMainRotateTime(2f);
                isFast = true;
            }
            else
            {
                slot.SetMainRotateTime(4);
                isFast = false;
            }
        }
