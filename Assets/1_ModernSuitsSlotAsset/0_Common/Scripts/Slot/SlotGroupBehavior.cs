...
namespace Mkey
{
    public class SlotGroupBehavior : MonoBehaviour
    {
        ...
        {
            return rayCasters.Length;
        }

        public RayCaster ReelElement(int index)
        {
            return rayCasters[index];
        }
...

        {
            ...
            tS.Add((callBack) =>  // main rotation part
            {
                ...
                var tween = SimpleTween.Value(gameObject, 0, -(angleX + outRotAngle + inRotAngle), mainRotTime)
                              .SetOnUpdate((float val) =>
                              {
                                  ...
                                  if(slotsStopped)
                                  {
                                      ...
                                      float xRotation = TilesGroup.transform.localRotation.eulerAngles.x;
                                      while ((int)(xRotation % 36) != 0)
                                      {
                                          val += 1f;
                                          TilesGroup.Rotate(val - oldVal, 0, 0);
                                          oldVal = val;
                                          xRotation = TilesGroup.transform.localRotation.eulerAngles.x;
                                      }
                                  }
                              })
                              .AddCompleteCallBack(() =>
