...
namespace Mkey
{
    ...
    public class SlotController : MonoBehaviour
    {
        ...
        [SerializeField]
        private WarningMessController NoCoinPrefab;
        #endregion prefabs
        ...
        public float winSpinLevelProgress = 3.0f;
        [Tooltip("Player level progress for win spin per win line")]
        public float DiamondLevelProgress = 2.0f;
        ...
        private WaitForSeconds wfs00_1;
        ...
        void Start()
        {
            ...
            wfs00_1 = new WaitForSeconds(0.01f);
            ...
            if (PlayerPrefs.GetInt("FreeSpins!") == 1)
            {
                controls.AddFreeSpins(7);
                controls.MaxBet_Click();
                controls.Fast_Click();
                PlayerPrefs.SetInt("FreeSpins!", 0);
                //if (!controls.Auto) controls.AutoPlayFreeSpins;
            }
        }
        ...
        public void SetMainRotateTime(float time)
        {
            mainRotateTime = time;
        }
        /// <summary>
        ...
        private void RunSlots()
        {
            ...
            {
                slotGroupsBeh[i].slotsStopped = false;
            }
            ...
            {
                if(NoCoinPrefab)
                    MGUI.ShowMessageWithYesNoCloseButton(NoCoinPrefab, null,"You have no money.", () => { }, () => { }, null);
                else
                    MGUI.ShowMessageWithYesNoCloseButton(null, "You have no money.", () => { }, null, null);
                controls.ResetAutoSpinsMode();
                controls.DisableStopButton();
                canSpin = true;
                return;
            }
            else
            {
                int piggyCoins = PlayerPrefs.GetInt("PiggyCoins", 0);
                int totalBet = controls.TotalBet/4;
                PlayerPrefs.SetInt("PiggyCoins", piggyCoins + totalBet);
            }
            gambleButton.gameObject.SetActive(false);
            StartCoroutine(RunSlotsAsync());

            if (spinCount % 10 == 0) //after every 10 spins
            {
        ...
        private IEnumerator RunSlotsAsync()
        {
            ...
            while (!fullRotated) yield return wfs00_1;  // wait 
            EndSpinEvent?.Invoke();
            ...
            if (winController.HasAnyWinn(ref hasLineWin, ref hasScatterWin, ref jackPotType))
            {
                ...
                //3c0 -----------------calc coins -------------------
                float winCoins = winController.GetWinCoins();
                int payMultiplier = winController.GetPayMultiplier();
                winCoins *= payMultiplier;
                winCoins *= MPlayer.StatusMultiplier;
                winCoins *= PlayerPrefs.GetInt("2xCoin!", 1); // if double coins booster active
                Debug.Log(controls.LineBet);
                Debug.Log("Total Bet: " + controls.TotalBet);
                Debug.Log("Coins Multiplier VIP: " + MPlayer.StatusMultiplier);
                if (useLineBetMultiplier) winCoins *= controls.TotalBet / 10; // winnings = betting amount/10 * payline payout
                MPlayer.SetWinCoinsCount(jackPotWinCoins + (int)winCoins);
                MPlayer.AddCoins((int)winCoins);
                if (winCoins > 0)
                {
                    if (!controls.HasFreeSpin)
                        gambleButton.gameObject.SetActive(true);
                    bigWin = (winCoins >= controls.TotalBet * 5 && MPlayer.UseBigWinCongratulation); // 5times the betting amount
                    if (!bigWin) MSound.PlayClip(0, winCoinsSound);
                    else
                    {
                        while (!MGUI.HasNoPopUp) yield return wfs0_1;  // wait for prev popup closing
                        MGUI.ShowMessage(BigWinPrefab, "", ((int)winCoins).ToString(), 3f, null); // aqua MGUI.ShowMessage(BigWinPrefab, " ", winCoins.ToString(), 3f, null);
                    }
                    FirebaseManager.Instance.UpdateTodayCoins((int)winCoins);
                }
                ...
                float xpIncreaseByBettingAmount = (controls.LineBet <= 10) ? controls.LineBet : 14;
                MPlayer.AddLevelProgress((float)((winSpinLevelProgress + (xpIncreaseByBettingAmount*0.5) )* PlayerPrefs.GetInt("2xXP!", 1)));
                ...
                winController.WinSymbolShow(winLineFlashing,
                       (windata) => //linewin
                       {
                           ...
                           if (windata != null)
                           {
                               Debug.Log("lineWin : " + windata.ToString() + windata.Symbols[0].GetSprite().name);
                               canSpin = true;
                           }
                       },
                       () => //scatter win
            ...
            else // lose
            {
                ...
                MPlayer.AddLevelProgress(loseSpinLevelProgress * PlayerPrefs.GetInt("2xXP!", 1));
            ...
            if (PlayerPrefs.GetInt("FirstDayPassed", 0) == 0)
                FirebaseManager.Instance.GetStartTimeAndDayFromFirebase();
        }
        ...
        private void CheckForDiamondSymbol()
        {
            for (int i = 0; i < slotGroupsBeh.Length; i++)
            {
                for (int j = 0; j < slotGroupsBeh[i].ReelLength(); j++)
                {
                    if (slotGroupsBeh[i].ReelElement(j).GetSymbol().GetComponent<SpriteRenderer>().sprite.name == "Diamond")
                    {
                        MPlayer.AddLevelProgress(DiamondLevelProgress * PlayerPrefs.GetInt("2xXP!", 1));
                        GetComponent<CoinCollectHelper>().SpawnCoins(slotGroupsBeh[i].ReelElement(j).transform.position, 5);
                    }
                }
            }
        }

        int[] rewardList = { 0, 1, 3, 5, 9, 10, 12, 14, 15, 16, 20 };
        private void RotateSlots(Action rotCallBack)
        {
            ...
            float winSpin = UnityEngine.Random.Range(0f, 1f);

            if (winSpin > 0.55 || PlayerPrefs.GetInt("FirstDayPassed", 0) == 1)
                for (int i = 0; i < slotGroupsBeh.Length; i++)
                {
                    int n = i;
                    int r = rands[i];
            ...
            else if (PlayerPrefs.GetInt("FirstDayPassed", 0) == 0)
            {
                int r = rewardList[UnityEngine.Random.Range(0, rewardList.Length)];
                for (int i = 0; i < slotGroupsBeh.Length; i++)
                {
                    int n = i;
                    //int r = rands[i];
        ...
        public void StopSlots()
        {
            for (int i = 0; i < slotGroupsBeh.Length; i++)
            {
                slotGroupsBeh[i].slotsStopped = true;
            }
