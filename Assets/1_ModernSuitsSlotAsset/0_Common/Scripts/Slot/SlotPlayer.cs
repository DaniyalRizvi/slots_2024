...
namespace Mkey
{
    public class SlotPlayer : MonoBehaviour
    {
        ...
        [Tooltip("Default gems at start")]
        [SerializeField]
        private int defGemsCount = 10;
        ...
        private string saveGemsKey = "mk_slot_gems"; // current gems
        ...
        private string saveStatusLevelKey = "mk_slot_status_progress"; // status level 
        private string saveStatusPointsProgressKey = "mk_slot_status_points_progress"; // status points 
        private string saveStatusMultiplierKey = "mk_slot_status_multiplier"; // status points 

        #endregion keys

        #region events
        public Action <int> ChangeCoinsEvent;
        public Action <int> ChangeGemsEvent;
        public Action<int> LoadCoinsEvent;
        public Action<int> LoadGemsEvent;
        ...
        private float defaultStatusMultiplier = 1;
        public float StatusMultiplier
        {
            get; private set;
        }
        private int defaultStatusPoints = 0;
        ...
        #region status
        /// <summary>
        /// Add status and save result
        /// </summary>
        /// <param name="count"></param>
        public void AddStatusPoints(int count)
        {
            SetStatusPoints(StatusPoints + count);
        }
        ...
        /// </summary>
        /// <param name="count"></param>
        public void SetStatusPoints(int count)
        {
            SetStatusPoints(count, true);
        }
        ...
        /// </summary>
        /// <param name="count"></param>
        private void SetStatusPoints(int count, bool raiseEvent)
        {
            count = Mathf.Max(0, count);
            bool changed = (StatusPoints != count);
            StatusPoints = count;

            Status = FindObjectOfType<VIPBenefits>().GetStatusLevel(StatusPoints);
            StatusMultiplier = FindObjectOfType<VIPBenefits>().GetStatusMultiplier(Status);
            if (SaveData && changed)
            {
                string key = saveStatusPointsProgressKey;
                PlayerPrefs.SetInt(key, StatusPoints);

                key = saveStatusLevelKey;
                PlayerPrefs.SetInt(key, Status);

                key = saveStatusMultiplierKey;
                PlayerPrefs.SetFloat(key, StatusMultiplier);
            }
        ...
        private void LoadStatus()
        {
            ...
            {
                string key = saveStatusPointsProgressKey;
                SetStatusPoints(PlayerPrefs.GetInt(key, defaultStatusPoints), false);

                //key = saveStatusLevelKey;
                //PlayerPrefs.SetInt(key, defaultStatus);

                //key = saveStatusMultiplierKey;
                //PlayerPrefs.SetFloat(key, defaultStatusMultiplier);

            }
            else
            {
                SetStatusPoints(defaultStatusPoints, false);
            }
        ...
        /// </summary>
        /// <param name="count"></param>
        private void SetGemsCount(int count, bool raiseEvent)
        {
            count = Mathf.Max(0, count);
            bool changed = (Gems != count);
            Gems = count;
            if (SaveData && changed)
            {
                string key = saveGemsKey;
                PlayerPrefs.SetInt(key, Gems);
            }
            if (changed && raiseEvent) ChangeGemsEvent?.Invoke(Gems);
        }
        ...
        private void LoadGems()
        {
            ...
            {
                string key = saveGemsKey;
                SetGemsCount(PlayerPrefs.GetInt(key, defGemsCount), false);
            }
            else
            {
                SetGemsCount(defGemsCount, false);
            }

            LoadGemsEvent?.Invoke(Gems);
        }
        #endregion gems
