...
namespace Mkey
{
    public class DailyRewardController : MonoBehaviour
    {
        ...
        private void Awake()
        {
            if (PlayerPrefs.GetInt("FirstTimeDailyReward", 0) == 0)
            {
                ResetData();
                PlayerPrefs.SetInt("FirstTimeDailyReward", 1);
            }
            //PlayerPrefs.SetInt("FirstTimeDailyReward", 0);
        }
        private void Update()
        {
            if (IsWork)
        ...
        private void TimePassedHandler(double initTime, double realyTime)
        {
            ...
            StartNewTimer();
           //FirebaseManager.Instance.ResetTodayCoins();
        }
