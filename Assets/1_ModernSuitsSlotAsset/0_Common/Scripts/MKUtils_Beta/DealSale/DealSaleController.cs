...
namespace Mkey
{
    public class DealSaleController : MonoBehaviour
    {
        ...
        private string resetDealWorkingTimerName = "reset_Timer";
        ...
        public void EndWorkingTimer()
        {
            //IsDealTime = true;
            //   SetIAP();
            IsDealTime = false;
            EndTimer(resetDealWorkingTimerName, workingTimeSpan.GetTimeSpan(), WorkingDealTickRestDaysHourMinSecHandler, WorkingDealTimePassedHandler, WorkingDealStartEvent);
        }
        ...
        private void EndTimer(string timerName, TimeSpan ts, Action<int, int, int, float> TickRestDaysHourMinSecAction, Action<double, double> TimePassedAction, Action StartAction)
        {
            if (debug) Debug.Log("start new timer: " + timerName);
            gTimer = new GlobalTimer(timerName, 0, 0, 0, 5);
            if (TickRestDaysHourMinSecAction != null) gTimer.TickRestDaysHourMinSecEvent += TickRestDaysHourMinSecAction;
            if (TimePassedAction != null) gTimer.TimePassedEvent += TimePassedAction;
            StartAction?.Invoke();
            IsWork = true;
